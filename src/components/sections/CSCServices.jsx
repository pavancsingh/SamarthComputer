    ...
    <section id="csc" className="py-16 md:py-24 bg-white border-b border-slate-200/60 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10 space-y-3">
          <div className="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-700 font-black text-xs px-4 py-1.5 rounded-full border border-emerald-200/80 shadow-xs">
        ...
        <div className="max-w-md mx-auto mb-12 relative">
          <input
            ...
            className="w-full pl-11 pr-4 py-3.5 bg-white border border-slate-300 rounded-2xl shadow-sm font-semibold text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all"
          />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {displayServices.map((item, idx) => (
              <div 
                ...
                className="bg-white rounded-3xl p-6 md:p-7 border border-slate-200/90 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between group"
              >
                <div className="space-y-3.5">
                  <div className="flex items-center justify-between">
                    <div className="w-11 h-11 rounded-2xl bg-rose-50 border border-rose-200/80 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-colors shadow-xs">
                  ...
                  <h3 className="font-extrabold text-lg text-slate-900 group-hover:text-primary transition-colors">
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/70 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {((isMarathi ? item.requiredDocsMr : item.requiredDocsEn) || []).slice(0, 3).map((doc, dIdx) => (
                        <span key={`doc-${dIdx}-${doc}`} className="bg-white border border-slate-200 text-slate-800 text-[11px] font-bold px-2.5 py-1 rounded-xl shadow-xs">
                          ✓ {doc}
                        </span>
                      ))}
                    </div>
                ...
                <div className="mt-6 flex gap-2 pt-4 border-t border-slate-100">
                  ...
                  >
                    ...
                    <span>{isMarathi ? '📞 कॉल करा' : 'Call Now'}</span>
                  </a>
                  ...
                  <a
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 bg-slate-900 hover:bg-emerald-600 text-white font-extrabold text-xs py-3 rounded-2xl shadow-sm transition-all"
                  >
        ...
        <div className="mt-12 text-center">
          <button
            ...
            className="inline-flex items-center gap-2 bg-slate-900 hover:bg-primary text-white px-8 py-3.5 rounded-2xl font-black text-sm shadow-md transition-all hover:scale-105 active:scale-95 group"
          >
            <span>{isMarathi ? 'सर्व ऑनलाईन सेवा पहा (View All Online Services)' : 'View All Online Services'}</span>
