...
/**
 * CSCServices Component - Google Stitch Design System
 * Live search and fast-action service cards for PAN Card, Income Certificate, Gazette, Shop Act, Passport, Udyam.
 */
export default function CSCServices({ lang = 'mr' }) {
  ...
  return (
    <section id="csc" className="py-20 bg-stitch-ivory border-b border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10 space-y-3">
          <div className="inline-flex items-center gap-1.5 bg-emerald-50 text-stitch-emerald font-extrabold text-xs px-4 py-1.5 rounded-full border border-emerald-200 shadow-stitch-sm">
            ...
            <span>{isMarathi ? 'महाऑनलाइन सीएससी व शासकीय सेवा' : 'MahaOnline CSC Government Services'}</span>
          </div>
          ...
          <p className={`text-slate-500 text-sm sm:text-base font-medium ${isMarathi ? 'marathi-text' : ''}`}>
            {isMarathi
              ? 'अचूक अर्ज भरणी, जलद सेवा आणि १००% शासकीय मान्यता. कागदपत्रे आणा आणि १५ मिनिटात काम करा.'
              : 'Accurate form filling, fast-track processing, and 100% government portal compliance.'}
          </p>
        </div>
        ...
        <div className="max-w-md mx-auto mb-12 relative">
          <input
            ...
            placeholder={isMarathi ? 'कोणती सेवेची गरज आहे? (उदा. पॅन कार्ड, दाखला)' : 'Search service (e.g. PAN Card, Gazette)'}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map(item => (
            ...
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="bg-slate-100 text-slate-700 text-[10px] font-bold px-3 py-1 rounded-full border border-slate-200">
                    {isMarathi ? (item.timelineMr || item.timeline_mr) : (item.timelineEn || item.timeline_en)}
                  </span>
                </div>
              ...
              <a
                href={`https://wa.me/919552345061?text=I%20want%20to%20apply%20for%20${encodeURIComponent(item.titleEn || item.titleMr)}`}
                target="_blank"
